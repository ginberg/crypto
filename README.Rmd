...
We can look at different metrics regarding currencies, like Price, Market Cap, number of coins, etc. To select which metric we are interested in, we will use another selectInput.
Add the code below after the first input, separated by a comma.
...
We all know that the cryptocurrency market is volatile, so it's very important to be able to select a certain period in time. Shiny has a dataRangeInput that we can use to specify a start and end date.

Before looking at the code below, can you find out how to create this dateRangeInput (hint: use ?dateRangeInput())?
...
### 7. [Server] Build the Reactive Data
...
#### 7.1 Reactive component getData



### 8. [Server] Build the graph
...
ggplot2: Add plots to your apps
